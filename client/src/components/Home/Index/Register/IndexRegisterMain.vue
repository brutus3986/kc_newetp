      ...
      <v-flex grow class="conWidth_left">
        <v-card flat lite pb-0>
          ...
          <registrationModify
            v-if="editYn"
            :editData="editData"
            :key="editData.jisu_id"
            @fn_refresh="fn_refresh"
            @fn_moveRegisterPage="fn_moveRegisterPage"
          ></registrationModify>
  ...
  export default {
    ...
    methods: {
      ...
      async fn_jisuRegister() {
        ...
        if(await vm.$root.confirmt.open('[신규지수등록]', '현재 작성중인 내용은 사라집니다. \n[신규지수등록] 화면으로 이동하시겠습니까?', {}, 2)) {
