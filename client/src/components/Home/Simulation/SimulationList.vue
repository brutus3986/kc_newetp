  ...
  <v-layout row wrap class="content_margin">
    <v-flex grow>
      <v-card flat class="creat_f">
        ...
        <span>
          <v-btn depressed outline color="primary" @click="fn_showCreateGroup();">그룹 생성</v-btn>
        </span>
                ...
                <tr v-for="( item, index ) in arr_simul_list" :key="'simul_' + index">
                  ...
                  <td class="txt_left">
                    ...
                    >
                      <div>
                        <v-icon
                          v-if="( typeof item.time_series_upload_yn != 'undefined' && item.time_series_upload_yn == '1')"
                    ...
                    <div class="tooltip">
                      ...
                      >
                        ...
                        <ul class="more_menu_w">
                          ...
                          <li
                            v-if="item.owner_yn == '0'"
                            @click="fn_apply_share_user_revoke_in_arr( item, index, 'grp' )"
                          >
                            <v-icon class="simul_more_btn">share</v-icon>공유해제
                          </li>
  ...
  export default {
    ...
    methods: {
        ...
        vm.fn_check_now_status().then(async function(e) {
          if(e && e.result) {
            ...
            if(["delete"].includes(v_status)) {
              ...
              if(await vm.$root.confirmt.open('[시나리오 그룹]', '[' + p_item.scen_name + '] 삭제 하시겠습니까?', {}, 2)) {
      ...
      fn_simul_delete(p_item, p_index) {
        ...
        vm.fn_check_now_status().then(function(e) {
          ...
          if(e && e.result) {
            ...
            if(await vm.$root.confirmt.open('[시나리오]', v_msg, {}, 2)) {
              if("Y" != vm.$root.confirmt.val) {
                return false;
              }
            }
      ...
      async fn_check_data(item, index) {
        var vm = this;
        if(item.grp_yn == "1") {
          if(await vm.$root.confirmt.open('확인', '그룹은 선택하실수 없습니다.', {}, 1)) {
          ...
          if(vm.arr_comp.length >= 10) {
            if(await vm.$root.confirmt.open('확인', '최대 10개 까지만 선택 가능합니다.', {}, 1)) {
              var v_index = _.findIndex(vm.arr_comp, function(o) {
                o = JSON.parse(o);
                return o.grp_cd == item.grp_cd && o.scen_cd == item.scen_cd;
              });
        ...
        return await new Promise(async function(resolve, reject) {
          if(vm.now_status == "ing") {
            if(["fn_show_rename", "fn_rename"].includes(vm.now_event)) {
              if(await vm.$root.confirmt.open('확인', '이름변경이 완료되지 않은 건이 존재합니다.', {}, 1)) {}
            }
            resolve({result: false});
          } else {
            resolve({result: true});
      ...
      async fn_copy_scenario(p_item, p_index) {
        ...
        if(await vm.$root.confirmt.open('확인', '[' + p_item.scen_name + '] 복사 하시겠습니까?', {}, 2)) {
          if("Y" != vm.$root.confirmt.val) {
            return false;
          }
        }
          ...
          if(p_item.grp_yn == '1') {
            ...
            if(p_item.simul_change_yn == "1") {
              if(p_item.owner_yn != "1") {
                if(await vm.$root.confirmt.open('확인', '시뮬레이션 결과와 시나리오 정보가 변동이 발생되었습니다. 소유자만 시나리오 화면으로 이동가능합니다.', {}, 1)) {
                  return false;
                }
              } else {
                if(await vm.$root.confirmt.open('확인', '시뮬레이션 결과와 시나리오 정보가 변동이 발생되었습니다. 시나리오 화면으로 이동하시겠습니까?', {}, 2)) {
                  if("Y" == vm.$root.confirmt.val) {
                    /* 수정정보를 보여준다. */
                    p_item.showSimulationId = 1;
                    vm.fn_showSimulation(p_item);
      ...
      async fn_rename_scenario(p_item, p_index) {
        ...
        if(v_obj && typeof v_obj.val() != "undefined") {
          if(v_obj.val() == "") {
            if(await vm.$root.confirmt.open('확인', '시나리오명 을 입력해 주세요.', {}, 1)) {
              v_obj.focus();
              return false;
            }
          }
      ...
      async fn_apply_share_user_revoke_in_arr(p_item, p_index, p_gubun = "scen") {
        ...
        if(!p_item || !p_item.grp_cd || !p_item.scen_cd) {
          if(vm.$root.confirmt.open('확인', "기본정보가 존재하지 않습니다.", {}, 1)) {}
          return false;
        }
        return await new Promise(async function(resolve, reject) {
          var p_param = {};
