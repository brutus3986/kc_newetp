      ...
      <v-card flat height="800px">
          ...
          <h3 class="headline w100" pb-0>
            ...
            <span class="btn_r">
              <button
                type="button"
                class="exceldown_btn"
                v-if="paramData.grp_cd && paramData.scen_cd && paramData.result_daily_yn == '1'"
                @click="fn_excelDown()"
              ></button>
            </span>
          </h3>
        ...
        <v-card class="register_wrap pt0" flat xs12 color="lighten-1">
          ...
          <v-layout row xs12>
            ...
            <v-flex xs2>
              <input type="text" class="upload-name" disabled :value="time_series_file_nm" />
            </v-flex>
            <v-flex xs2>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      vm.fn_getInitSimul();
      /* 포트폴리오 파일 영역 */
      this.$refs.timeSeriesUpload.addEventListener("change", async function(evt) {
    ...
    methods: {
      ...
      async fn_excelDown() {
        ...
        if(!vm.paramData.grp_cd || !vm.paramData.scen_cd) {
          if(await vm.$root.confirmt.open('[엑셀파일 유형확인]', "신규등록은 다운로드 하실수 없습니다.", {}, 1)) {}
          return false;
        }
        var options = {
          skipHeader: true,
                    ...
                    try {
                      if(response && response.data) {
                        ...
                        if(!response.data.result) {
                          ...
                          excelInfo.arrHeaderNm = ["DATE", "지수"];
                          excelInfo.arrHeaderKey = ["fmt_F12506", "INDEX_RATE"];
                          excelInfo.arrColsInfo = [{width: 20}, {width: 45}];
