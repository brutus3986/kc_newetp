    ...
    <v-flex grow>
      ...
      <v-card flat class="bot_pad1">
        ...
        <h4>
          ...
          <span class="btn_r">
            <v-btn
              v-if="method_gubun == 'getScenInGrpCd'"
              small
              flat
              icon
  ...
  export default {
    ...
    computed: {
      ...
      fn_sort_arr_result_daily01: function() {
        var vm = this;
        return _.orderBy(vm.arr_result_daily01, ["F12506"], ["desc"]);
      },
            ...
            if(e1 && e1.result) {
              try {
                ...
                vm.arr_result_daily01_header.forEach(function(item, index, array) {
                  ...
                  v_arr_show_column.push({
                    "data": item.grp_cd + "_" + item.scen_cd + "_INDEX_RATE",
                    "orderable": false,
                    'className': 'dt-body-right'
                  });
                  v_arr_show_column.push({
                    "data": item.grp_cd + "_" + item.scen_cd + "_RETURN_VAL",
                    "orderable": false,
                    'className': 'dt-body-right'
                  });
              ...
              try {
                ...
                if(vm.arr_result_anal02.length == 0) {
                  ...
                  vm.arr_result_anal02_header.forEach(function(item, index, array) {
                    ...
                    v_arr_show_column.push({
                      "data": item.grp_cd + "_" + item.scen_cd,
                      "orderable": false,
                      'className': 'dt-body-right'
                    });
                    v_arr_show_columnDef.push({
    ...
    methods: {
       ...
       */
      async fn_getSimulDailyInArrCd(p_param = {
        grp_cd: "",
        arr_scen_in_grp: []
      }) {
        var vm = this;
       ...
       * 2019-08-14  bkLove(촤병국)
       */
      async fn_getSimulAnal01InArrCd(p_param = {
        grp_cd: "",
        arr_scen_in_grp: []
      }) {
        var vm = this;
       ...
       * 2019-08-14  bkLove(촤병국)
       */
      async fn_getSimulAnal02InArrCd(p_param = {
        grp_cd: "",
        arr_scen_in_grp: []
      }) {
        var vm = this;
            ...
            return await new Promise(function(resolve, reject) {
              try {
                ...
                dataWS['!ref'] = excel.utils.encode_range({
                  s: {c: 0,r: 0},
                  e: {
                    c: (arr_row2.length * vm.arr_result_daily01_header.length) + arr_row2.length,
      ...
      async fn_open_share_modal(p_item, p_index) {
        ...
        if(typeof vm.owner_all_yn != "undefined" && vm.owner_all_yn == "0") {
          if(await vm.$root.confirmt.open('확인', '공유받은 시나리오가 한개 이상 포함되어 있어 공유가 불가합니다.', {}, 1)) {}
          return false;
        }
        var v_param = {};
        v_param = p_item;
