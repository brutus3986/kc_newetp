              ...
              <div class="left_w">
                ...
                <span class="toggle2">
                  <v-btn-toggle v-model="importance_method_cd" class="toggle_01">
                    <v-btn
                      v-for="(item, index) in arr_importance_method_cd"
              ...
              <v-card flat>
                ...
                <div class="text-xs-center mt-3">
                  ...
                  <v-btn
                    depressed
                    color="primary"
                    @click.stop="fn_runBacktestWithSaveBasicInfo()"
                  >백테스트 실행</v-btn>
                </div>

                <MastPopup02
                  v-if="MastModalFlag"
                  @selectedItem="fn_getSelectedItem"
                  @closeMastModal="fn_closeMastModal"
                ></MastPopup02>
              </v-card>
            </v-flex>
          </v-layout>
  ...
  export default {
    ...
    mounted() {
      ...
      $('#table01 tbody').on('change', "input[name='importance']", function() {
        ...
        vm.fn_setRebalancePortfolioObj({
          p_gubun: "tr",
          p_obj: tr
        });
        vm.fn_resetErrorMessage();
      ...
      /* 포트폴리오 파일 영역 */
      this.$refs.portfolioFile.addEventListener("change", async function(evt) {
        var vm = this;
    ...
    methods: {
      ...
      async fn_refresh() {
        var vm = this;
        if(await vm.$root.confirmt.open('[시뮬레이션]', '입력된 내용이 모두 초기화 됩니다. 그래도 진행하시겠습니까?', {}, 2)) {
      ...
      async fn_getInitSimul() {
        ...
        if(vm.paramData && Object.keys(vm.paramData).length > 0 && vm.paramData.grp_cd && vm.paramData.scen_cd) {
          vm.detail_init_yn = "Y";
      ...
      async fn_setImportanceMethodCd(importance_method_cd) {
        ...
        return await new Promise(function(resolve, reject) {
          if(vm.rebalancePortfolioObj && vm.rebalance_date && typeof vm.rebalancePortfolioObj[vm.rebalance_date] != "undefined" && vm.rebalancePortfolioObj[vm.rebalance_date]) {
                ...
                if(v_item && Object.keys(v_item).length > 0) {
                  ...
                  for(var j = 0; j < Object.keys(v_item).length; j++) {
                    ...
                    totalObj[v_key].importance = Number(
                      (Number(totalObj[v_key].importance) + Number(v_sub_item.importance)).toFixed(5)); /* (합계) 비중 */
                  ...
                  if(["2", "3"].includes(p_importance_method_cd)) {
                    ...
                    same_rate = (Math.floor((Number(totalObj[v_key].same_rate_sum) / Number(totalObj[v_key].length)) * 100000) / 100000); /* 동일 가중 비율 */
                    v_temp_importance = 0;
                    v_inx = 0;
                    for(var j = 0; j < Object.keys(v_item).length; j++) {
                      var v_sub_key = Object.keys(v_item)[j];
                      ...
                      /* 시총비중인 경우 */
                        ...
                        v_temp_importance = Number(Math.floor((Number(v_sub_item.F15028) / Number(totalObj[v_key].F15028)) * 10000000) / 100000);
                      }
                      totalObj[v_key].importance = Number(
                        (Number(totalObj[v_key].importance) + Number(v_temp_importance)).toFixed(5)); /* (합계) 비중 */
      ...
      async fn_getJongmokData(rowIndex = 0, v_obj = {}) {
        var vm = this;
        if(v_obj.val() == "") {
          if(await vm.$root.confirmt.open('확인', '종목코드를 입력해 주세요.', {}, 1)) {
            return false;
          }
          return false;
        }
        if(v_obj.val().length < 6) {
          if(await vm.$root.confirmt.open('확인', '종목코드를 6자리 이상 입력해 주세요.', {}, 1)) {
            return false;
          }
          return false;
        }
                ...
                if(response.data) {
                  ...
                  if(!response.data.result) {
                    ...
                    if(!dataList || dataList.length == 0) {
                      if(await vm.$root.confirmt.open('확인', '종목코드(' + v_obj.val() + ')가 존재하지 않습니다.', {}, 1)) {
                        resolve({result: false});
                      }
                    } else if(dataList.length > 1) {
                      if(await vm.$root.confirmt.open('확인', '종목코드(' + v_obj.val() + ')가 여러건 존재합니다.', {}, 1)) {
                        resolve({result: false});
                      }
                    } else if(dataList.length == 1) {
                      resolve({result: true,
          ...
          table01.find("tbody tr").each(function(inx, rowItem) {
            ...
            if(typeof v_F16013.val() != "undefined") {
              if(v_F16013.val() != "") {
                ...
                total.importance = Number(Number(Number(Number(total.importance).toFixed(5)) + Number(Number(util.NumtoStr(v_importance.val())).toFixed(5))).toFixed(5)); /* (합계) 비중 */
              }
            }
          });
        }
      ...
      fn_runBacktestWithSaveBasicInfo() {
        ...
        vm.fn_copySelectedDateToRebalanceAll({
          p_gubun: "first",
          ...
          for(var i = 0; i < Object.keys(vm.rebalancePortfolioObj).length; i++) {
            ...
            if(Object.keys(v_sub_item).length > 1) {
              ...
              for(var j = 0; j < arrKey.length; j++) {
                ...
                v_importance = Number(Number(Number(Number(v_importance).toFixed(5)) + Number(v_sub_item2.importance)).toFixed(5)); /* (합계) 비중 */
            ...
            if(response && response.data) {
              ...
              if(!response.data.result) {
                ...
                if(mastInfo && Object.keys(mastInfo).length > 0) {
                  ...
                  /* 주식수 구분 */
                  vm.stock_gubun = (typeof mastInfo.stock_gubun == "undefined" || mastInfo.stock_gubun == null || mastInfo.stock_gubun == "" ? "1" : mastInfo.stock_gubun);
            ...
            if(response && response.data) {
              ...
              if(!response.data.result) {
                ...
                if(typeof v_rebalancePortfolio != "undefined" && Object.keys(v_rebalancePortfolio).length > 0) {
                  ...
                  /* trIndex 순으로 정렬하여 노출 */
                  dataList = _.orderBy(dataList, ["trIndex"], ["asc"]);
                  ...
                  if(vm.rebalance_cycle_cd == "") {
                    if(vm.old_start_year != vm.start_year) {
                      if(Number(vm.excel_start_year) > Number(vm.start_year)) {
                        if(await vm.$root.confirmt.open('[시뮬레이션]', vm.excel_start_year + " 년도 이후로만 선택해 주세요.", {}, 1)) {
            ...
            "headers": {
              ...
              try {
                if(response.data) {
                  if(!response.data.result) {
                    var errorList = [];
                    if((typeof response.data.count_check != "undefined" && !response.data.count_check) || (typeof response.data.record_check != "undefined" && !response.data.record_check)) {
