        ...
        <v-layout>
          <v-flex xs12>
            ...
            <ul>
              ...
              <li v-if="etpBasic.F34241 == 'K'">
                iNAV=
                <span class="txt_point">①전일NAV</span>×(1+
                <span class="txt_point3">③기초지수등락율</span>×
                <span class="txt_point2">②배율</span>)
              </li>
              <li v-else>
                <span v-if="etpBasic.F34240 == 'H'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point3">③기초지수등락율</span>×
                  <span class="txt_point2">②배율</span>)
                </span>
                <span v-else-if="etpBasic.F34240 == 'F'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point3">③기초지수등락율</span>×
                  <span class="txt_point2">②배율</span>)×(
                  <span class="txt_point4">④적용환율/기준환율</span>)
                </span>
                <span v-else-if="etpBasic.F34240 == 'A'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point3">③기초지수등락율</span>×
                  <span class="txt_point2">②배율</span>×
                ...
                <span v-else-if="etpBasic.F34240 == 'T'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point3">③기초지수등락율</span>×
                  <span class="txt_point2">②배율</span>)
                ...
                <span v-else-if="etpBasic.F34240 == 'K'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+((1+
                  <span class="txt_point3">③기초지수등락율</span>)
                  <br />×
                  <span class="txt_point4">④적용환율/기준환율</span>-1)×
                  <span class="txt_point2">②배율</span>)
                </span>
                <span v-else-if="etpBasic.F34240 == 'I'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+((1+
                  <span class="txt_point3">③기초지수등락율</span>)×
                  <span class="txt_point4">④적용환율/기준환율</span>-1)
                  <br />×
                  <span class="txt_point2">②배율</span>) ×(1+전일등락율×
                  <span class="txt_point2">②배율</span>)
                </span>
                <span v-else-if="etpBasic.F34240 == 'J'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point3">③기초지수등락율</span>×
                  <span class="txt_point2">②배율</span>-예상배당수익률)
                </span>
                <span v-else-if="etpBasic.F34240 == 'G'">
                  iNAV=
                  <span class="txt_point">①전일NAV</span>×(1+
                  <span class="txt_point2">②배율</span>×
                  <span class="txt_point3">③지수등락율</span>)×(환율보정계수/기준환율)
                </span>
              </li>
            </ul>
          ...
          <v-flex xs12 class="sumul_card_w ver3">
            ...
            <ul class="bot_line2">
              ...
              <li>
                ...
                <div v-else>
                  <li class="align_r">{{formatNumber(formatDigit(F15007, 4))}}</li>
                  <br />
                  <span class="float_r">{{etpBasic.R_BASIC_INDEX_DATE}}</span>
                </div>
              </li>
        ...
        <v-layout>
          <v-flex xs12 class="sumul_card_w ver3">
            ...
            <ul class="bot_line1">
              ...
              <li v-else class="align_r text_red">{{formatNumber(F30819)}}</li>
            </ul>
            <ul class="bot_line2">
              ...
              <li class="align_r" v-else>{{formatNumber(F30824)}}</li>
            </ul>
            <ul
              v-if="etpBasic.F34240 == 'F' || etpBasic.F34240 == 'A' || etpBasic.F34240 == 'K' || etpBasic.F34240 == 'I' "
            >
            ...
            <ul class="result ver2 bot_line1">
              ...
              <li class="text_red align_r" v-if="etpBasic.F30818 >= 0">
                <b>{{formatNumber(formatDigit(etpBasic.F15301, 2))}}</b>
                <br />
                <span class="float_r">{{formatDigit(etpBasic.F30818, 2)}}%</span>
              </li>
            ...
            <ul class="result">
              ...
              <li class="align_r text_red" v-if="iNavRate >=0 ">
                <b>{{formatNumber(formatDigit(iNav, 2))}}</b>
                <br />
                <span class="float_r">{{formatDigit(iNavRate, 2)}}%</span>
              </li>
  ...
  export default {
    ...
    data() {
      return {
        ...
        /* 변동률 */
        F15004_1: 0,
        /* (매매기준율(적용환율)/장전매매기준율(기준환율)) */
        F15004_2: 0,
        /* (매매기준율(적용환율)-장전매매기준율(기준환율))/장전매매기준율(기준환율) */
        F34374: 0,
        /* 전일ETP기초지수등락율 */
        F18101: 0,
        /* 예상배당수익률 : 배당율 */
            ...
            */
            if(vm.etpBasic.F34240 == 'H') {
              ...
              */
              ...
              vm.iNav = vm.F03329 * (1 + vm.F30823 * vm.F18453) * (vm.F30819 / vm.F30824);
              /* 
              ...
              */
              ...
              vm.iNav = vm.F03329 * (1 + ((1 + vm.F30823) * vm.F30819 / vm.F30824 - 1) * vm.F18453);
              /* 
              T.복합배율 :  TIGER 차이나A레버리지(합성)
              iNAV=전일NAV*(1+기초지수등락율*배율)*(1+(매매기준율-장전매매기준율)/장전매매기준율*배율)               
              */
            } else if(vm.etpBasic.F34240 == 'T') {
              vm.iNav = vm.F03329 * (1 + vm.F30823 * vm.F18453) * (1 + (vm.F30819 - vm.F30824) / vm.F30824 * vm.F18453);
              /*
              ...
              */
              ...
              vm.iNav = vm.F03329 * (1 + ((1 + vm.F30823) * vm.F30819 / vm.F30824 - 1) * vm.F18453) * (1 + vm.F34374 * vm.F18453);
              /* 
              ...
              */
              ...
              vm.iNav = vm.F03329 * (1 + vm.F30823 * vm.F18453 - vm.F18101);
              /* 
              G.  KODEX 미국채10년 선물 ETF
              iNAV = 전일NAV * (1+지수등락율*배율)*(환율보정계수 / 장전매매기준율)
              */
            } else if(vm.etpBasic.F34240 == 'G') {
              vm.iNav = vm.F03329 * (1 + vm.F30823 * vm.F18453) * (vm.F33128 / vm.F30824);
            }
          }
          /* iNav 등락률 */
          vm.iNavRate = ((vm.iNav / vm.F03329) - 1) * 100;
          vm.iNav = vm.iNav;
          vm.iNavRate = vm.iNavRate;
          vm.F30823 = vm.formatDigit(vm.F30823 * 100, 5); /* 등락률 */
          vm.F15004 = vm.formatDigit(vm.F15004, 5); /* 변동률 */
          // (ETP계산유형: F, A, K, I)매매기준율 /장전 매매 기준율
          vm.F15004_1 = vm.formatDigit(vm.F30819 / vm.F30824, 5);
          // (ETP계산유형: T)(매매기준율 - 장전 매매 기준율)/ 장전매매기준율
