              ...
              <v-card flat class="listset_pop">
                ...
                <v-card flat class="pdf_mody_w">
                  <v-toolbar card prominent>
                    ...
                    >일괄변경 (엑셀업로드)</v-btn>
                    <input type="file" name="pdfUpload" ref="pdfUpload" style="display:none;" />
                  ...
                  >
                    <ul>
                      <li v-for="(item, index) in arr_show_error_message" :key="index">{{item}}</li>
                    </ul>
                  </div>
                </v-card>
            ...
            <v-card class="mx-auto" v-show="step==2">
              ...
              <v-card flat class="listset_pop">
                ...
                <v-card flat class="pdf_context">
                  <div class="pdf_coment text-xs-center">* 추가로 변경하실 ETF 종목이 있다면 [추가 변경]을 선택하세요.</div>
...
<script>
  ...
  export default {
    ...
    mounted: function() {
      ...
      /* PDF 업로드 파일 영역 */
      this.$refs.pdfUpload.addEventListener("change", async function(evt) {
        var vm = this;
    ...
    methods: {
      ...
      async fn_setSearchData(dataList, dataJson) {
        ...
        if(dataList && dataList.length > 1) {
          vm.status = 1;
          if(await vm.$root.confirmt.open('확인', '구성종목코드(' + dataJson.codeVal + ')가 여러건 존재합니다.', {}, 1)) {
            dataJson.codeVal = "";
            dataJson.initYn = "Y";
            vm.fn_setInitData(vm, dataJson);
            vm.status = 0;
        ...
        if(duplCheck.length > 0) {
          vm.status = 1;
          if(await vm.$root.confirmt.open('확인', '구성종목코드(' + dataJson.codeVal + ')가 이미 존재합니다.', {}, 1)) {
            dataJson.codeVal = "";
            dataJson.initYn = "Y";
            vm.fn_setInitData(vm, dataJson);
            vm.status = 0;
      ...
      fn_getEtpOperPdfModify(searchParam) {
        ...
        vm.fn_getNowDate(searchParam).then(async function(e1) {
          ...
          if(!vm.search_date) {
            vm.status = 1;
            if(await vm.$root.confirmt.open('확인', '최근일자 정보가 존재하지 않습니다.', {}, 1)) {
      ...
      async fn_getJongmokData(dataJson) {
        ...
        if(dataJson.status == "insert") {
          if(!dataJson.codeVal || dataJson.codeVal.length == 0) {
            vm.status = 1;
      ...
      fn_stepCheck(step) {
        ...
        if(step == 1) {
          ...
          if(!vm.allDataList || vm.allDataList.length == 0) {
      ...
      async fn_addEtfOperPdfModify() {
        ...
        if(!vm.txtAddEtpCode || vm.txtAddEtpCode.length == 0) {
          vm.result.flag = false;
      ...
      fn_setStatus(tableData, nowData, rowIndex) {
        ...
        if(nowData.name == "F16499") {
          if(tableData.status == "insert") {
             ...
             */
            if(Number(tableData.F16499_prev) == Number(nowData.F16499) && Number(tableData.F34840_prev) == Number(tableData.F34840)) {
        ...
        /* 액면금액 */
          ...
          if(tableData.status == "insert") {
             ...
             */
            if(Number(tableData.F34840_prev) == Number(nowData.F34840) && Number(tableData.F16499_prev) == Number(tableData.F16499)) {
              table.cell(rowIndex, 9).data("normal").draw();
              vm.dataList[rowIndex].status = "normal";
            } else {
              table.cell(rowIndex, 9).data("modify").draw();
      ...
      fn_uploadPdf(file) {
        ...
        var p_param = {};
        vm.arr_show_error_message = [];
        if(vm.etpBasic.F16012 == "undefined" || vm.etpBasic.F16012 == "") {
          vm.arr_show_error_message.push("ETF 코드가 존재하지 않습니다.");
          return false;
        }
        if(vm.etpBasic.F16013 == "undefined" || vm.etpBasic.F16013 == "") {
          vm.arr_show_error_message.push("ETF 코드가 존재하지 않습니다.");
          return false;
        }
        if(vm.etpBasic.F16583 == "undefined" || vm.etpBasic.F16583 == "") {
          vm.arr_show_error_message.push("ETF 코드가 존재하지 않습니다.");
          return false;
        }
